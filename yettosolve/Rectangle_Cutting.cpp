        ...
        for(int i=1;i<501;i++){
            for(int j=i+1;j<501;j++){
                if(i==j)dp[i][j]=0;
                    ...
                    ll mini = min(i,j);
                    ll maxi = max(i,j);
                    for(int k=1;k<=maxi/2;k++){
                        ll tp = k;
                        ll rem = maxi-k;
                        dp[i][j] = min(dp[min(tp,mini)][max(tp,mini)]+dp[min(rem,mini)][max(rem,mini)]+1,dp[i][j]);
                    }

                    for(int k=1;k<=mini/2;k++){
                        ll tp = k;
                        ll rem = mini-k;
                        dp[i][j] = min(dp[min(tp,maxi)][max(tp,maxi)]+dp[min(rem,maxi)][max(rem,maxi)]+1,dp[i][j]);
                    }  
                } 
            }
        }
