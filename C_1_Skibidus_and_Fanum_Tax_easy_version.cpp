...
bool check(vl &a,vl &c,ll curr,ll n){
    for(int i=1;i<n;i++){
        if(a[i]<curr and c[i]<curr){
            return false;
        }
        else if(a[i]<curr and c[i]>=curr){
            curr = c[i];
        }
        else if(c[i]<curr and a[i]>=curr){
            curr = a[i];
        }
        else{
            curr = min(a[i],c[i]);
...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        ll s1 = a[0];
        ll s2 = c[0];
        bool ans1 = check(a,c,s1,n);
        bool ans2 = check(c,a,s2,n);
        if(ans1 or ans2){
            cout<<"YES"<<nl;
        }
        else{
