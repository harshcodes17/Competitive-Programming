    ...
    ll helper(ll idx,vl &v,vector<vector<ll>>&dp,ll k){
        // cout << "idx: " << idx << " prev: " << prev << " k: " << k << " score: " << score << nl;    
        if(k==0 || idx==v.size()){ 
            return 0;
        }
        if(dp[idx][k] != -1){
        ...
        for(int i=1;i<=k;i++){
            ...
            sum+=v[idx+i];
            mini = min(mini,v[idx+i]);
            ll leave = sum-(mini*(i+1))+helper(idx+i+1,v,dp,k-i);
            choose = max(choose,leave);
        }
        return dp[idx][k] = max(leave,choose);
    }
