...
//To find modulo inverse, call powermod(A,M-2,M)
    ...
    // ll f1(ll i,ll j,ll sum,vector<vector<ll>> &mat , vector<vector<ll>>&dp1){
    ...
    //         return dp1[i][j];
    //     }
    //     ll right = f1(i,j+1,sum+mat[i][j],mat,dp1);
    //     ll down = f1(i+1,j,sum+mat[i][j],mat,dp1);
    //     return dp1[i][j] = max(right,down);
    // }

    // ll f2(ll i,ll j,ll sum,vector<vector<ll>> &mat , vector<vector<ll>>&dp2){
    //     if(i<0 || j>=mat[0].size()){
    //         return 0;
    //     }
    ...
    //         return dp2[i][j];
    //     }
    //     ll right = f2(i,j+1,sum+mat[i][j],mat,dp2);
    //     ll up = f2(i-1,j,sum+mat[i][j],mat,dp2);
    //     return dp2[i][j] = max(right,up);
    // }
    ...
    int32_t main()
    {
        ...
        auto solve = [&] () {
            ...
            for(int i=0;i<n;i++){
                for(int j=0;j<m;j++){
                    ll temp = dp1[i][j]+dp2[i][j]+dp3[i][j]+dp4[i][j]-4LL*mat[i][j];
                    ans = max(ans,temp);
                }
            }
            cout<<ans<<nl;
