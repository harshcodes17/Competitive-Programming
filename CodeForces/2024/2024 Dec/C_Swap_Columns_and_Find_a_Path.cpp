...
int32_t main()
{
        ...
        auto solve = [&]()
    {
        ll n;
        cin >> n;
        vector<vector<ll>> mat(2, vector<ll>(n));
        for (int i = 0; i < 2; i++)
        {
            for (int j = 0; j < n; j++)
            {
        ...
        vector<vector<ll>> mat2(2, vector<ll>(n));
        for(int i=0;i<left.size();i++){
            mat2[0][i]=mat[0][left[i]];
            mat2[1][i]=mat[1][left[i]];
        }
        for(int i=0;i<right.size();i++){
            mat2[0][i+left.size()]=mat[0][right[i]];
            mat2[1][i+left.size()]=mat[1][right[i]];
        }
        ll ans = 0;
        for(int i=0;i<n;i++){
            ans+=max(mat2[0][i],mat2[1][i]);
        }
        // for(int i=0;i<2;i++){
        //     for(int j=0;j<n;j++){
        //         cout<<mat2[i][j]<<" ";
        ...
        ll addition = -1e18;
        for(int i=0;i<left.size();i++){
            addition = max(addition,mat[1][left[i]]);
        }
        // cout<<"left:"<<left<<nl;
        // cout<<"right:"<<right<<nl;
        for(int i=0;i<right.size();i++){
            addition = max(addition,mat[0][right[i]]);
        }
