...
ll f(int i,int j,string &s,string &t,string &c,vector<vector<ll>> &dp){
    ...
    ll res = 1e10;
    if(i<s.size()){
        res = min(res,f(i+1,j,s,t,c,dp)+(s[i]!=c[pos]));
    }
    if(j<t.size()){
        res = min(res,f(i,j+1,s,t,c,dp)+(t[j]!=c[pos]));
    }
    return dp[i][j] = res;
}
