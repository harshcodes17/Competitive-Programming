...
ll helper(ll idx,ll color,vvl &dp){
    if(idx==n){
        return 0;
    }
    if(dp[idx][color]!=-1){
    ...
    ll opA = helper(idx+1,v[idx-1],dp)+(color==v[idx]?hot[v[idx]]:cold[v[idx]]);
    ll opB = helper(idx+1,color,dp)+(v[idx-1]==v[idx]?hot[v[idx]]:cold[v[idx]]);
    return dp[idx][color] = min(opA,opB);
}
...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        vector dp(n,vector(k+1,-1LL));
        ll ans = cold[v[0]]+helper(1,0,dp);
        cout<<ans<<endl;
