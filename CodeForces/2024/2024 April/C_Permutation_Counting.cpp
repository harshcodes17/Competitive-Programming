...
public:
    ...
    void BuildTree(ll v[], ll i, ll low, ll high)
    {
        if (low == high)
        {
            seg[i] = v[low];
...
void inputArr(ll n, ll v[])
{
    for (ll i = 0; i < n; i++)
        cin >> v[i];
}

void inputArr(ll n, vector<ll> &v)
{
    ll x;
    for (ll i = 0; i < n; i++)
    {
...
void printArr(ll n, vector<ll> &v)
{
    for (ll i = 0; i < n; i++)
        cout << v[i] << " ";
    cout << nl;
