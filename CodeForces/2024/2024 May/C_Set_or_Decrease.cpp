...
bool check(ll mid,vl &v,ll max_sum,ll arraysum){

    if(arraysum - mid <= max_sum){
        return true;
    }
    ll n = v.size();
    ll sum = arraysum;
    for(ll i=n-1;i>=max(1LL,(n-mid));i--){
        sum-=v[i];
        ll opsleft = mid - (n-i);
        ll tempsum = sum - v[0] + (n-i+1)*(v[0]-opsleft);
        if(tempsum<=max_sum){
            return true;
        }
    }
...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        ll ans=1e10;
        // cout<<v<<nl;
        ll arraysum = accumulate(all(v),0LL);
        ll l = 0;
        ll r = 1e10;
        while(l<=r){
            ll mid = (l+r)/2;
            if(check(mid,v,k,arraysum)){
                ans = mid;
                r = mid-1;
            }
            else{
