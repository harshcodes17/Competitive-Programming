...
bool check(double mid , vector<double> &pos, vector<double> &spe){
   
    double currl = -1e18 ,currr = 1e18;

    for(int i=0;i<pos.size();i++){
        double position = pos[i];
        double speed = spe[i];

        double rangel = position - (mid*speed);
        double ranger = position + (mid*speed);

        currl = max(currl,rangel);
        currr = min(currr,ranger);
