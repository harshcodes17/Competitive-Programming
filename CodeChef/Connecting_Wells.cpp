...
ll dist(pair<ll, ll>& well1, pair<ll, ll>& well2) {
    return max(abs(well1.first - well2.first), abs(well1.second - well2.second));
}
...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        while(pq.size()){
            ...
            for(int i=0;i<n;i++){
                if(!vis[i]){
                    ll xaxis = abs(wells[u].first-wells[i].first);
                    ll yaxis = abs(wells[u].second-wells[i].second);
                    ll c = 0;
                    if(xaxis==0){
                        c = (yaxis+1)/2;
                    }
